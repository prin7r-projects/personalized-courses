/**
 * [READING_LIST_NOWPAYMENTS_IPN] POST /api/webhooks/nowpayments
 *
 ...
 *    and trigger course generation; until then we log the verified event so
 *    the deploy host's journalctl captures it.
 *
...
export async function POST(request: Request) {
  ...
  // Stub — when apps/app ships, this becomes a DB write + course-generation
  // trigger. For now journalctl is the audit trail. We do not log the full
  // payload because it can carry pay_address-style identifiers.
  console.log(
    `[READING_LIST_NOWPAYMENTS_IPN] verified=true order_id=${orderId} status=${status} paid=${paid}`
  );
